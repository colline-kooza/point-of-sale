        ...
        <Link href="/dashboard/">
             <Button variant="outline" size="icon" className="h-7 w-7">
              ...
              <div className="hidden items-center gap-2 md:ml-auto md:flex">
                ...
                {
                  isLoading?(
                    <Button variant='outline' disabled={isLoading}  className="w-full flex gap-2 items-center bg-slate-950 text-white">
                    <PiSpinnerGapLight className="animate-spin"/>{initialData?"Updating":"Creating"}
                </Button>
                  ):(
                    <Button size="sm">{initialData?"Update Table":"Create Table"}</Button>
                  )
                }
              </div>
            </div>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="grid gap-6 col-span-1 lg:col-span-2">
                     <TextInput label="Table Title" name="title" register={register} errors={errors} type="text" />
                     <TextInput label="Table Number" name="tableNumber" register={register} errors={errors} type="number" />
                    </div>
                  </CardContent>
                </Card>
              </div>
            ...
            <div className="flex items-center justify-center gap-2 md:hidden">
              ...
              {/* <Button type="submit" size="sm">Save Table</Button> */}
              {
          isLoading ? (
            <Button variant='outline' disabled={isLoading}  className="w-full flex gap-2 items-center bg-slate-950 text-white">
            <PiSpinner className="animate-spin"/> {initialData?"Updating":"Creating Table"}
           </Button>
           )  : (
            <Button variant='outline' disabled={isLoading}  className="w-full flex gap-2 items-center bg-slate-950 text-white">
            <PiSpinner className="animate-spin"/> {initialData?"Updating":"Create Table"}
           </Button>
           )
           }
